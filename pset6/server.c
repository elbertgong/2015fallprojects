...
char* indexes(const char* path)
{
    ...
    // 10 extra characters to contain the phrase "/index.php\0"
    char* pathphp = malloc(strlen(path) + 11);
    ...
    strcpy(pathphp, path);
    strcat(pathphp, "/index.php"); 
    
    // if index.php exists along this path
    if (access(pathphp, F_OK) == 0)
    {
        return pathphp;
    }
    
    // the same thing, just for html
    char* pathhtml = malloc(strlen(path) + 12);
    if (pathhtml == NULL)
    {
        error(500);
    }
    strcpy(pathhtml, path);
    strcat(pathhtml, "/index.html");
    if (access(pathhtml, F_OK) == 0)
    {
        free(pathphp);
        return pathhtml;
    }
    
    // if neither index.php or index.html exist
    free(pathphp);
    free(pathhtml);
    return NULL;
}
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // we're going to be reading byte by byte into a dynamic "buffer"
    BYTE* buffer = malloc(1);
    *length = 0;
    
    // read from file into buffer until EOF reached
    while (fread(buffer + *length, 1, 1, file) != 0)
    {
        
        // keeping track of length
        *length += 1;
        
        // reallocing memory in anticipation of the next iteration of fread
...
const char* lookup(const char* path)
{
    ...
    // mimetype will point to the "." at the end of "path"
    char* mimetype = strrchr(path, '.');
    if (mimetype == NULL)
    {
        return NULL;
    }
    else if (strcasecmp(mimetype, ".css") == 0)
    {
        return "text/css";
    }
    else if (strcasecmp(mimetype, ".html") == 0)
    {
        return "text/html";
    }
    else if (strcasecmp(mimetype, ".gif") == 0)
    {
        return "image/gif";
    }
    else if (strcasecmp(mimetype, ".ico") == 0)
    {
        return "image/x-icon";
    }
    else if (strcasecmp(mimetype, ".jpg") == 0)
    {
        return "image/jpeg";
    }
    else if (strcasecmp(mimetype, ".js") == 0)
    {
        return "text/javascript";
    }
    else if (strcasecmp(mimetype, ".php") == 0)
    {
        return "text/x-php";
    }
    else if (strcasecmp(mimetype, ".png") == 0)
    {
        return "image/png";
    }
    else
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // checking whether the line ends with /r/n
    if (*(line + m - 2) != '\r' || *(line + m - 1) != '\n')
    {
        error(400);
        return false;
    }
    
    // counting up the number of spaces in the line
    for (int i = 0; i < m; i++)
    {
        if (*(line + i) == ' ')
        {
        ...
        // checking for any " in the request target
        if (spaces == 1 && *(line + i) == '"')
        {
    ...
    // checking whether the number of spaces is correct
    if (spaces != 2)
    {
        error(400);
        return false;
    ...
    // checking whether the method is correct
    ...
    // this pointer will point to the first character in the request target
    char* reqtarg = strchr(line, ' ') + 1;
    
    // checking whether the request target begins with /
    if (*reqtarg != '/')
    {
        error(501);
        return false;
    ...
    // checking whether the HTTP version is correct
    char* version;
    ...
    // initializing abs_path and query
    for (int x = 0; x < LimitRequestLine; x++)
    {
        *(abs_path + x) = '\0';
        *(query + x) = '\0';
    }
    
    // stepping through "reqtarg", char by char, and storing those chars
    // in the appropriate string, until a space is encountered
    int j;
    for (j = 0; *(reqtarg + j) != ' '; j++)
    {
        
        // when we encounter a ?, we need to start writing into query
        if (*(reqtarg + j) == '?')
        {
            *(abs_path + j) = '\0';
            int k;
            
            // write into query until you encounter a space
            for (k = j + 1; *(reqtarg + k) != ' '; k++)
            {
                *(query + k - j - 1) = *(reqtarg + k);
            }
            *(query + k) = '\0';
            return true;
        }
        
        *(abs_path + j) = *(reqtarg + j);
    }
    
    // if a question mark was never encountered
    *(abs_path + j) = '\0';
    *query = '\0';
    return true;
}
